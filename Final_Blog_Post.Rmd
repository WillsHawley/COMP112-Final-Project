...
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=popularity),fill="dark red",bins=22) +
  theme_minimal() +
  labs(x="Popularity",y="Count",title="Distribution of Popularity Values")
```
...
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=acousticness),fill="dark red",bins=40) +
  theme_minimal() +
  labs(x="Acousticness",y="Count",title="Distribution of Acousticness Values")
```
...
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=danceability),fill="dark red",bins=40) +
  theme_minimal() +
  labs(x="Danceability",y="Count",title="Distribution of Danceability Values")
```
...
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=duration_ms),fill="dark red",bins=100) +
  theme_minimal() +
  labs(x="Duration",y="Count",title="Distribution of Duration Values in Milliseconds")
```
...
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=energy),fill="dark red",bins=30) +
  theme_minimal() +
  labs(x="Energy",y="Count",title="Distribution of Energy Values")
```
...
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=instrumentalness),fill="dark red",bins=50) +
  theme_minimal() +
  labs(x="instrumentalness",y="Count",title="Distribution of Instrumentalness Values")
```
...
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=liveness),fill="dark red") +
  theme_minimal() +
  labs(x="Liveness",y="Count",title="Distribution of Liveness Values")
```


* Loudness = The overall loudness of a track in decibels (dB). Loudness values are averaged across the entire track. Loudness is the quality of a sound that is the primary psychological correlate of physical strength (amplitude). Values typical range between -60 and 0 db.

```{r,echo=FALSE}
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=loudness),fill="dark red") +
  theme_minimal() +
  labs(x="Loudness",y="Count",title="Distribution of Loudness Values")
```
...
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=speechiness),fill="dark red") +
  theme_minimal() +
  labs(x="Speechiness",y="Count",title="Distribution of Speechiness Values")
```
...
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=tempo),fill="dark red") +
  theme_minimal() +
  labs(x="Tempo",y="Count",title="Distribution of Tempo Values")
```
...
Spotify_Big %>% 
  ggplot() +
  geom_histogram(aes(x=valence),fill="dark red",bins=25) +
  theme_minimal() +
  labs(x="Valence",y="Count",title="Distribution of Valence Values")
```
...
* Here we see that there is major variability based on the genre. Clearly, there are more popular genres than others. We used plots such as this when designing our app in order to identify the best ways to display the information we wanted to convey. 
...
* This is the second kind of data we wanted to display. In looking at a two variable comparison by genre, we were more able to look at trends within given genres, rather than simply relying on single variable comparisons by genre. 

### The App

* On Plot 1 we utilized a Hex Plot
...
It was difficult to prove any causual relationship because the data was difficult to use and our inexperience with statistics. So we had to find a way to display some sort of relationship in a clear and concise way. Using the hex plots and box plots we found a good way to show several different aspects of the data. When experimenting with the data we were able to pick out different relationships using our plots in conjunction. We wanted to know what factors affect the popularity of a song. However, there are other factors that may influence popularity that aren't in the data set that are worth taking into account when trying to infer effects on popularity. These include popularity of the artist, and genre in current culture, and the lyrics in the song. Althoguh, after completing our project we discovered that the variables that affect popularity are the valence and the energy of the song. 
